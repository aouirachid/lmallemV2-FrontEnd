...
@Component({
  selector: 'app-delivred-order',
  standalone: true,
  imports: [RouterModule, NgFor, NgIf, DataTablesModule, CommonModule],
  templateUrl: './delivred-order.component.html',
  styleUrl: './delivred-order.component.css',
})
export class DelivredOrderComponent {
    ...
    this.orderService.getOrders().subscribe({
      next: (data: any) => {
        ...
        if (!data || !Array.isArray(data)) {
          ...
          this.Orders = data.filter((order: any) => order.orderStatus == 3);
        }
